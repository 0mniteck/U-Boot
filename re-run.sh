...
BUILDK="--buildkitd-config $PWD/../Includes/buildkitd.toml --driver=docker-container --driver-opt=\"network=host,image=moby/buildkit:v0.25.1-rootless\" --name U-Boot-Builder"
set -x
if [ "$TARGETS" != "" ]; then
  echo "TARGET: $TARGETS"
